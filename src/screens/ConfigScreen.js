import { AlertDialog, Menu, Popover, Tooltip, Modal, Box, VStack, Text, Pressable, HStack, Icon, Divider, Center, useColorModeValue, Switch, Button, Badge, ScrollView, useColorMode } from 'native-base';
...
const ConfigurationScreen = () => {
    ...
    const bgColor = colorMode === "light" ? colors.background.light:colors.background.default;
    const textColor = useColorModeValue('light.text.900', 'dark.text.100');
    const iconColor = useColorModeValue('dar.icon.200', 'white');
    const boxColor = colorMode==='light' ? "muted.100":colors.darkAccent[400];
    const borderColor = colorMode==='light'?'muted.300':colors.primary[900]
    return (
        <Center flex={1} bg={bgColor}>
            ...
            >
                ...
                <VStack space={4}>
                    ...
                    <Divider background={colorMode==='light'? "muted.200":colors.highlight[800]} 
                    ...
                    <Pressable onPress={() => setIsMenuModalOpen(true)}>
                        <Text fontSize="md" color={colorMode==='light'?"blue.500":colors.primary[400]}>
